...
SELF-BLEND — ⛔ ВЫКЛЮЧЕН ПО УМОЛЧАНИЮ. yaromat 2026-07-16: «зеркало даёт эффект КАЛЕЙДОСКОПА,
   вообще не то». hflip даёт осевую симметрию — она читается декоративным калейдоскопом, а не
   двойной экспозицией. Реверс времени тоже мимо: в середине клипа совпадает с оригиналом
   (замерено: 6.6 на краях → 1.0 в центре, двоение пульсирует).
   Если возвращаться к идее — пробовать СДВИГ ПО ВРЕМЕНИ (копия с задержкой N кадров = шлейф,
   тот же дрейф, без симметрии и без схлопывания). НЕ включать без явного запроса.
